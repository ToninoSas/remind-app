...
export default function AnagraficaPaziente({ data, patientId }) {
  ...
  const handleSave = async () => {
    ...
    if (res.success) {
      ...
      // RESET PASSWORD DOPO IL SALVATAGGIO, IN MODO CHE NON RESTI VISIBLE NEL FORM PER UNA VOLTA DOPO LA MODIFICA
      setEditData((prev) => ({
        ...prev,
        password: "",
      }));
      ...
      {!isEditing ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-6">
            <div>
              <p className="text-[16px] font-black text-slate-700 uppercase tracking-[0.2em] mb-2">
                Profilo Clinico
              </p>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-slate-800 font-bold">{data.info.Email}</p>
              <p className="text-[12px] text-slate-700 mt-2 font-black italic">
                Iscritto il: {data.info.Created_At}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white p-10 rounded-[3rem] shadow-2xl max-w-sm w-full text-center animate-in zoom-in duration-300 border border-slate-300">
            ...
            <p className="text-slate-800 text-sm font-bold mb-10 leading-relaxed italic">
              Il profilo di <strong>{data.info.First_Name}</strong> verrà
              disattivato.
            </p>
