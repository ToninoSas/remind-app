"use server";
import db from "@/lib/db"; // Import dell'istanza del database
import bcrypt from "bcryptjs";

import { cookies } from "next/headers";
import { redirect } from "next/navigation";

const SALT_ROUNDS = 10; // Numero di round per l'hashing, più è alto più è sicuro ma più lento (10 è un buon compromesso per applicazioni web)
...
export async function logoutAction() {
  ...
  // Reindirizziamo alla pagina di login dopo il logout
  redirect("/login");
}
