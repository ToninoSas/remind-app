...
const patientCreateSchema = z.object({
  ...
  patologia: z.enum(patologieEnum, "Patologia non valida"),
...
export async function createPatientAction(data, caregiverId) {
  ...
  if (!parsed.success) {
    const firstError = Object.values(parsed.error.formErrors.fieldErrors)[0][0];
...
export async function updatePatientAction(patientId, utenteId, data) {
  ...
  const { nome, cognome, email, patologia, descrizione, password  } = parsed.data;
